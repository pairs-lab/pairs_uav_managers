...
public:
  /*//{ constructor */
  TfMappingOrigin(const rclcpp::Node::SharedPtr &node, std::shared_ptr<pairs_lib::ParamLoader> param_loader,
    ...
    const std::string yaml_prefix = "pairs_uav_managers/transform_manager/mapping_origin_tf/";
    ...
    sh_mapping_odom_lat_ =
        pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, "/" + ch_->uav_name + "/" + lateral_topic_, &TfMappingOrigin::callbackMappingOdomLat, this);

    if (orientation_topic_ != lateral_topic_) {
      sh_mapping_odom_rot_ = pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(shopts, "/" + ch_->uav_name + "/" + orientation_topic_.c_str(),
                                                                                               &TfMappingOrigin::callbackMappingOdomRot, this);
    }

    if (altitude_topic_ != lateral_topic_) {
      sh_mapping_odom_alt_ = pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, "/" + ch_->uav_name + "/" + altitude_topic_.c_str(),
                                                                                 &TfMappingOrigin::callbackMappingOdomAlt, this);
    }
    /*//}*/

    ph_map_delay_ = pairs_lib::PublisherHandler<pairs_msgs::msg::Float64Stamped>(node_, "map_delay_out");
...
private:
  ...
  pairs_lib::PublisherHandler<pairs_msgs::msg::Float64Stamped> ph_map_delay_;
  ...
  pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry> sh_mapping_odom_lat_;
  ...
  void callbackMappingOdomLat(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "TfMappingOrigin::callbackMappingOdomLat", ch_->scope_timer.logger, ch_->scope_timer.enabled);

    const double hdg_mapping_old = pairs_lib::AttitudeConverter(msg->pose.pose.orientation).getHeading();
    ...
    if (got_mapping_odom_rot_ && got_mapping_odom_alt_) {
      ...
      tf2_rot = pairs_lib::AttitudeConverter(rot_tmp.quaternion);

      // Obtain heading from orientation
      double hdg = 0;
      try {
        hdg = pairs_lib::AttitudeConverter(rot_tmp.quaternion).getHeading();
      }
      catch (...) {
      ...
      tf2::Matrix3x3 rot_mat = pairs_lib::AttitudeConverter(Eigen::AngleAxisd(hdg_mapping_old - hdg, Eigen::Vector3d::UnitZ()));

      // Transform the mavros orientation by the rotation matrix
      geometry_msgs::msg::Quaternion new_orientation = pairs_lib::AttitudeConverter(tf2::Transform(rot_mat) * tf2_rot);
      ...
      pairs_msgs::msg::Float64Stamped map_delay_msg;
  ...
  pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped> sh_mapping_odom_rot_;
  ...
  pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry> sh_mapping_odom_alt_;
