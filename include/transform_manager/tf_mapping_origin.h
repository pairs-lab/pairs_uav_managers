...
public:
  /*//{ constructor */
  TfMappingOrigin(ros::NodeHandle nh, std::shared_ptr<pairs_lib::ParamLoader> param_loader, const std::shared_ptr<pairs_lib::TransformBroadcaster>& broadcaster,
    ...
    sh_mapping_odom_lat_ =
        pairs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, "/" + ch_->uav_name + "/" + lateral_topic_, &TfMappingOrigin::callbackMappingOdomLat, this);

    if (orientation_topic_ != lateral_topic_) {
      sh_mapping_odom_rot_ = pairs_lib::SubscribeHandler<geometry_msgs::QuaternionStamped>(shopts, "/" + ch_->uav_name + "/" + orientation_topic_.c_str(),
                                                                                         &TfMappingOrigin::callbackMappingOdomRot, this);
    }

    if (altitude_topic_ != lateral_topic_) {
      sh_mapping_odom_alt_ = pairs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, "/" + ch_->uav_name + "/" + altitude_topic_.c_str(),
                                                                           &TfMappingOrigin::callbackMappingOdomAlt, this);
    }
    /*//}*/

    ph_map_delay_ = pairs_lib::PublisherHandler<pairs_msgs::Float64Stamped>(nh_, "map_delay_out", 10);
...
private:
  ...
  pairs_lib::PublisherHandler<pairs_msgs::Float64Stamped> ph_map_delay_;
  ...
  pairs_lib::SubscribeHandler<nav_msgs::Odometry> sh_mapping_odom_lat_;
  ...
  void callbackMappingOdomLat(const nav_msgs::Odometry::ConstPtr msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("TfMappingOrigin::callbackMappingOdomLat", ch_->scope_timer.logger, ch_->scope_timer.enabled);
    ...
    if (got_mapping_odom_rot_ && got_mapping_odom_alt_) {
      ...
      geometry_msgs::Quaternion new_orientation = pairs_lib::AttitudeConverter(tf2::Transform(rot_mat) * tf2_rot);
      ...
      pairs_msgs::Float64Stamped map_delay_msg;
  ...
  pairs_lib::SubscribeHandler<geometry_msgs::QuaternionStamped> sh_mapping_odom_rot_;
  ...
  pairs_lib::SubscribeHandler<nav_msgs::Odometry> sh_mapping_odom_alt_;
