...
public:
  /*//{ constructor */
  TfSource(const std::string& name, ros::NodeHandle nh, std::shared_ptr<pairs_lib::ParamLoader> param_loader,
           const std::shared_ptr<pairs_lib::TransformBroadcaster>& broadcaster, const std::shared_ptr<estimation_manager::CommonHandlers_t> ch,
    ...
    if (name != "dummy") {
      ...
      sh_tf_source_odom_ = pairs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, full_topic_odom_, &TfSource::callbackTfSourceOdom, this);
      if (tf_from_attitude_enabled_) {
        sh_tf_source_att_ = pairs_lib::SubscribeHandler<geometry_msgs::QuaternionStamped>(shopts, full_topic_attitude_, &TfSource::callbackTfSourceAtt, this);
      }
    ...
    for (auto frame_id : republish_in_frames_) {
      republishers_.push_back(
          std::make_pair(frame_id, pairs_lib::PublisherHandler<nav_msgs::Odometry>(nh_, full_topic_odom_ + "/" + frame_id.substr(0, frame_id.find("_origin")))));
    }
...
private:
  ...
  std::vector<std::pair<std::string, pairs_lib::PublisherHandler<nav_msgs::Odometry>>> republishers_;

  pairs_lib::SubscribeHandler<nav_msgs::Odometry>               sh_tf_source_odom_;
  pairs_lib::SubscribeHandler<geometry_msgs::QuaternionStamped> sh_tf_source_att_;
  ...
  void callbackTfSourceOdom(const nav_msgs::Odometry::ConstPtr msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::callbackTfSourceOdom", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void callbackTfSourceAtt(const geometry_msgs::QuaternionStamped::ConstPtr msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::callbackTfSourceAtt", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishTfFromOdom(const nav_msgs::OdometryConstPtr& odom) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::publishTfFromOdom", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishTfFromAtt(const geometry_msgs::QuaternionStampedConstPtr& msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::publishTfFromAtt", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishUtmTf(const std::string& frame_id) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::publishUtmTf", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishWorldTf(const std::string& frame_id) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::publishWorldTf", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  /* republishInFrame() //{*/
  void republishInFrame(const nav_msgs::OdometryConstPtr& msg, const std::string& frame_id, pairs_lib::PublisherHandler<nav_msgs::Odometry>& ph) {

    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::republishInFrame", ch_->scope_timer.logger, ch_->scope_timer.enabled);
