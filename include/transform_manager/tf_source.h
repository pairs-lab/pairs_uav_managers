...
public:
  /*//{ constructor */
  TfSource(const std::string &name, const rclcpp::Node::SharedPtr &node, std::shared_ptr<pairs_lib::ParamLoader> param_loader,
    ...
    sh_tf_source_odom_ = pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, full_topic_odom_, &TfSource::callbackTfSourceOdom, this);

    if (tf_from_attitude_enabled_) {
      sh_tf_source_att_ = pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(shopts, full_topic_attitude_, &TfSource::callbackTfSourceAtt, this);
    }
    ...
    for (auto frame_id : republish_in_frames_) {
      republishers_.push_back(std::make_pair(
          frame_id, pairs_lib::PublisherHandler<nav_msgs::msg::Odometry>(node_, full_topic_odom_ + "/" + frame_id.substr(0, frame_id.find("_origin")))));
    }
...
private:
  ...
  std::vector<std::pair<std::string, pairs_lib::PublisherHandler<nav_msgs::msg::Odometry>>> republishers_;

  pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>               sh_tf_source_odom_;
  pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped> sh_tf_source_att_;
  ...
  void callbackTfSourceOdom(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::callbackTfSourceOdom", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void callbackTfSourceAtt(const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::callbackTfSourceAtt", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishTfFromOdom(const nav_msgs::msg::Odometry::ConstSharedPtr &odom) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::publishTfFromOdom", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishTfFromAtt(const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr &msg) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::publishTfFromAtt", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishUtmTf(const std::string &frame_id) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::publishUtmTf", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void publishWorldTf(const std::string &frame_id) {
    ...
    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::publishWorldTf", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  void republishInFrame(const nav_msgs::msg::Odometry::ConstSharedPtr &msg, const std::string &frame_id,
                        pairs_lib::PublisherHandler<nav_msgs::msg::Odometry> &ph) {

    pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::republishInFrame", ch_->scope_timer.logger, ch_->scope_timer.enabled);
