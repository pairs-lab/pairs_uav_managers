...
void Estimator::publishDiagnostics() const {
  ...
  pairs_msgs::msg::EstimatorDiagnostics msg;
...
tf2::Vector3 Estimator::getAccGlobal(const pairs_msgs::msg::EstimatorInput::ConstSharedPtr &input_msg, const double hdg) {
...
std::optional<double> Estimator::getHeadingRate(const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg) {
  ...
  try {
    hdg_rate = pairs_lib::AttitudeConverter(odom_msg->pose.pose.orientation).getHeadingRate(odom_msg->twist.twist.angular);
  }
  catch (...) {
