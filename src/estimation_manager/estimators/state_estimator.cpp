...
/*//{ getUavState() */
std::optional<pairs_msgs::msg::UavState> StateEstimator::getUavState() {
  ...
  return pairs_lib::get_mutexed(mtx_uav_state_, uav_state_);
}
/*//}*/

/*//{ getInnovation() */
nav_msgs::msg::Odometry StateEstimator::getInnovation() const {
  return pairs_lib::get_mutexed(mtx_innovation_, innovation_);
}
/*//}*/

/*//{ getPoseCovariance() */
std::vector<double> StateEstimator::getPoseCovariance() const {
  return pairs_lib::get_mutexed(mtx_covariance_, pose_covariance_.values);
}
/*//}*/

/*//{ getTwistCovariance() */
std::vector<double> StateEstimator::getTwistCovariance() const {
  return pairs_lib::get_mutexed(mtx_covariance_, twist_covariance_.values);
}
/*//}*/
...
void StateEstimator::publishUavState() const {
  ...
  auto uav_state = pairs_lib::get_mutexed(mtx_uav_state_, uav_state_);
...
void StateEstimator::publishOdom() const {
  ...
  auto odom = pairs_lib::get_mutexed(mtx_odom_, odom_);
...
void StateEstimator::publishCovariance() const {
  ...
  auto pose_cov  = pairs_lib::get_mutexed(mtx_covariance_, pose_covariance_);
  auto twist_cov = pairs_lib::get_mutexed(mtx_covariance_, twist_covariance_);
...
void StateEstimator::publishInnovation() const {
  ...
  auto innovation = pairs_lib::get_mutexed(mtx_innovation_, innovation_);
...
std::optional<geometry_msgs::msg::Quaternion> StateEstimator::rotateQuaternionByHeading(const geometry_msgs::msg::Quaternion &q, const double hdg) const {
  ...
  try {
    tf2::Quaternion tf2_q = pairs_lib::AttitudeConverter(q);

    // Obtain heading from quaternion
    double q_hdg = 0;
    q_hdg        = pairs_lib::AttitudeConverter(q).getHeading();

    // Build rotation matrix from difference between new heading and quaternion heading
    tf2::Matrix3x3 rot_mat = pairs_lib::AttitudeConverter(Eigen::AngleAxisd(hdg - q_hdg, Eigen::Vector3d::UnitZ()));

    // Transform the quaternion orientation by the rotation matrix
    geometry_msgs::msg::Quaternion q_new = pairs_lib::AttitudeConverter(tf2::Transform(rot_mat) * tf2_q);
    return q_new;
  }
  catch (...) {
...
bool StateEstimator::isCompatibleWithHwApi(const pairs_msgs::msg::HwApiCapabilities::ConstSharedPtr &hw_api_capabilities) const {
