...
/*//{ getUavState() */
std::optional<pairs_msgs::UavState> StateEstimator::getUavState() {
...
std::optional<geometry_msgs::Quaternion> StateEstimator::rotateQuaternionByHeading(const geometry_msgs::Quaternion& q, const double hdg) const {
  ...
  try {
    ...
    geometry_msgs::Quaternion q_new = pairs_lib::AttitudeConverter(tf2::Transform(rot_mat) * tf2_q);
    return q_new;
  }
  catch (...) {
...
bool StateEstimator::isCompatibleWithHwApi(const pairs_msgs::HwApiCapabilitiesConstPtr& hw_api_capabilities) const {
