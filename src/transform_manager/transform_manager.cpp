...
private:
  ...
  void                                                callbackHeightAgl(const pairs_msgs::Float64Stamped::ConstPtr msg);

  pairs_lib::SubscribeHandler<pairs_msgs::HwApiAltitude> sh_altitude_amsl_;
  void                                               callbackAltitudeAmsl(const pairs_msgs::HwApiAltitude::ConstPtr msg);
  ...
  pairs_lib::SubscribeHandler<pairs_msgs::RtkGps> sh_rtk_gps_;
  void                                        callbackRtkGps(const pairs_msgs::RtkGps::ConstPtr msg);
...
void TransformManager::onInit() {
  ...
  broadcaster_ = std::make_shared<pairs_lib::TransformBroadcaster>();

  ch_->transformer = std::make_shared<pairs_lib::Transformer>(nh_, getPrintName());
  ch_->transformer->retryLookupNewest(true);

  pairs_lib::ParamLoader param_loader(nh_, getPrintName());
  ...
  for (size_t i = 0; i < tf_source_names_.size(); i++) {
    ...
    auto source_param_loader = std::make_shared<pairs_lib::ParamLoader>(nh_, "TransformManager/" + tf_source_name);
  ...
  for (int i = 0; i < int(estimator_names_.size()); i++) {
    ...
    auto estimator_param_loader = std::make_shared<pairs_lib::ParamLoader>(nh_, "TransformManager/" + estimator_name);
  ...
  if (mapping_origin_tf_enabled) {
    ...
    auto mapping_param_loader = std::make_shared<pairs_lib::ParamLoader>(nh_, "TransformManager/mapping_origin_tf");
  ...
  sh_uav_state_ = pairs_lib::SubscribeHandler<pairs_msgs::UavState>(shopts, "uav_state_in", &TransformManager::callbackUavState, this);

  sh_height_agl_ = pairs_lib::SubscribeHandler<pairs_msgs::Float64Stamped>(shopts, "height_agl_in", &TransformManager::callbackHeightAgl, this);

  sh_altitude_amsl_ = pairs_lib::SubscribeHandler<pairs_msgs::HwApiAltitude>(shopts, "altitude_amsl_in", &TransformManager::callbackAltitudeAmsl, this);

  sh_hw_api_orientation_ =
      pairs_lib::SubscribeHandler<geometry_msgs::QuaternionStamped>(shopts, "orientation_in", &TransformManager::callbackHwApiOrientation, this);

  if (utm_source_name_ == "rtk" || utm_source_name_ == "rtk_garmin") {
    sh_rtk_gps_ = pairs_lib::SubscribeHandler<pairs_msgs::RtkGps>(shopts, "rtk_gps_in", &TransformManager::callbackRtkGps, this);
  } else {
    sh_gnss_ = pairs_lib::SubscribeHandler<sensor_msgs::NavSatFix>(shopts, "gnss_in", &TransformManager::callbackGnss, this);
  }
  /*//}*/
...
void TransformManager::callbackUavState(const pairs_msgs::UavState::ConstPtr msg) {

  if (!is_initialized_) {
    return;
  }

  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("TransformManager::publishFcuUntilted", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::callbackHeightAgl(const pairs_msgs::Float64Stamped::ConstPtr msg) {

  if (!is_initialized_) {
    return;
  }

  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("TransformManager::callbackHeightAgl", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::callbackAltitudeAmsl([[maybe_unused]] const pairs_msgs::HwApiAltitude::ConstPtr msg) {
...
void TransformManager::callbackGnss(const sensor_msgs::NavSatFix::ConstPtr msg) {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("TransformManager::callbackGnss", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
/*//{ callbackRtkGps() */
void TransformManager::callbackRtkGps(const pairs_msgs::RtkGps::ConstPtr msg) {

  if (!is_initialized_) {
    return;
  }

  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("TransformManager::callbackRtkGps", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::publishLocalTf() {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(getPrintName() + "::publishLocalTf", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_managers::transform_manager::TransformManager, nodelet::Nodelet)
