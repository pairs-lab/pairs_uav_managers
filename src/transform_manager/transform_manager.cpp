...
class TransformManager : public pairs_lib::Node {
...
private:
  ...
  bool callbackSetWorldOrigin(const std::shared_ptr<pairs_msgs::srv::ReferenceStampedSrv::Request>  request,
  ...
  std::shared_ptr<pairs_lib::TransformBroadcaster>       broadcaster_;
  ...
  void                                                      callbackHeightAgl(const pairs_msgs::msg::Float64Stamped::ConstSharedPtr msg);

  pairs_lib::SubscriberHandler<pairs_msgs::msg::HwApiAltitude> sh_altitude_amsl_;
  void                                                     callbackAltitudeAmsl(const pairs_msgs::msg::HwApiAltitude::ConstSharedPtr msg);

  pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped> sh_hw_api_orientation_;
  ...
  pairs_lib::SubscriberHandler<pairs_msgs::msg::RtkGps> sh_rtk_gps_;
  void                                              callbackRtkGps(const pairs_msgs::msg::RtkGps::ConstSharedPtr msg);
...
TransformManager::TransformManager(rclcpp::NodeOptions options) : pairs_lib::Node("estimation_manager", options) {
...
void TransformManager::initialize() {
  ...
  error_publisher_ = std::make_unique<pairs_lib::errorgraph::ErrorPublisher>(node_, clock_, "TransformManager", "main");
  ...
  broadcaster_        = std::make_shared<pairs_lib::TransformBroadcaster>(node_);
  ...
  const std::string yaml_prefix = "pairs_uav_managers/transform_manager/";
  ...
  param_loader.loadParam("pairs_uav_managers/world_origin/units", world_origin_units_);
  ...
  if (Support::toLowercase(world_origin_units_) == "utm") {
    ...
    RCLCPP_ERROR(node_->get_logger(), "[%s]: pairs_uav_managers/world_origin/units must be (\"UTM\"|\"LATLON\"). Got '%s'", getPrintName().c_str(),
  ...
  param_loader.loadParam("pairs_uav_managers/estimation_manager/state_estimators", estimator_names_);
  ...
  for (size_t i = 0; i < tf_source_names_.size(); i++) {
    ...
    auto source_param_loader = std::make_shared<pairs_lib::ParamLoader>(node_, "TransformManager/" + tf_source_name);
  ...
  for (int i = 0; i < int(estimator_names_.size()); i++) {
    ...
    auto estimator_param_loader = std::make_shared<pairs_lib::ParamLoader>(node_, "TransformManager/" + estimator_name);
  ...
  if (mapping_origin_tf_enabled) {
    ...
    auto mapping_param_loader = std::make_shared<pairs_lib::ParamLoader>(node_, "TransformManager/mapping_origin_tf");
  ...
  sh_uav_state_ = pairs_lib::SubscriberHandler<pairs_msgs::msg::UavState>(shopts, "~/uav_state_in", &TransformManager::callbackUavState, this);

  sh_height_agl_ = pairs_lib::SubscriberHandler<pairs_msgs::msg::Float64Stamped>(shopts, "~/height_agl_in", &TransformManager::callbackHeightAgl, this);

  sh_altitude_amsl_ = pairs_lib::SubscriberHandler<pairs_msgs::msg::HwApiAltitude>(shopts, "~/altitude_amsl_in", &TransformManager::callbackAltitudeAmsl, this);

  sh_hw_api_orientation_ =
      pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(shopts, "~/orientation_in", &TransformManager::callbackHwApiOrientation, this);

  if (utm_source_name_ == "rtk" || utm_source_name_ == "rtk_garmin") {
    sh_rtk_gps_ = pairs_lib::SubscriberHandler<pairs_msgs::msg::RtkGps>(shopts, "~/rtk_gps_in", &TransformManager::callbackRtkGps, this);
  } else {
    sh_gnss_ = pairs_lib::SubscriberHandler<sensor_msgs::msg::NavSatFix>(shopts, "~/gnss_in", &TransformManager::callbackGnss, this);
  }
  /*//}*/
...
void TransformManager::callbackUavState(const pairs_msgs::msg::UavState::ConstSharedPtr msg) {

  if (!is_initialized_) {
    return;
  }

  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "TransformManager::publishFcuUntilted", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::callbackHeightAgl(const pairs_msgs::msg::Float64Stamped::ConstSharedPtr msg) {

  if (!is_initialized_) {
    return;
  }

  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "TransformManager::callbackHeightAgl", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::callbackAltitudeAmsl([[maybe_unused]] const pairs_msgs::msg::HwApiAltitude::ConstSharedPtr msg) {
...
void TransformManager::callbackGnss(const sensor_msgs::msg::NavSatFix::ConstSharedPtr msg) {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "TransformManager::callbackGnss", ch_->scope_timer.logger, ch_->scope_timer.enabled);

  double out_x;
  double out_y;

  pairs_lib::UTM(msg->latitude, msg->longitude, &out_x, &out_y);
...
void TransformManager::callbackRtkGps(const pairs_msgs::msg::RtkGps::ConstSharedPtr msg) {

  if (!is_initialized_) {
    return;
  }

  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "TransformManager::callbackRtkGps", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
bool TransformManager::callbackSetWorldOrigin(const std::shared_ptr<pairs_msgs::srv::ReferenceStampedSrv::Request>  request,
                                              const std::shared_ptr<pairs_msgs::srv::ReferenceStampedSrv::Response> response) {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "TransformManager::callbackSetWorldOrigin", ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  if (request->header.frame_id.find("latlon_origin") != std::string::npos) {
    ...
    pairs_lib::UTM(lat, lon, &world_origin.x, &world_origin.y);
...
void TransformManager::publishFcuUntiltedTf(const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr msg) {
  ...
  try {
    heading = pairs_lib::AttitudeConverter(msg->quaternion).getHeading();
  }
  catch (...) {
  ...
  const Eigen::Matrix3d odom_pixhawk_R = pairs_lib::AttitudeConverter(msg->quaternion);
  const Eigen::Matrix3d undo_heading_R = pairs_lib::AttitudeConverter(Eigen::AngleAxis(-heading, Eigen::Vector3d(0, 0, 1)));

  const tf2::Quaternion q     = pairs_lib::AttitudeConverter(undo_heading_R * odom_pixhawk_R);
  ...
  tf.transform.rotation      = pairs_lib::AttitudeConverter(q_inv);
...
void TransformManager::publishLocalTf() {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, getPrintName() + "::publishLocalTf", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
RCLCPP_COMPONENTS_REGISTER_NODE(pairs_uav_managers::transform_manager::TransformManager)
