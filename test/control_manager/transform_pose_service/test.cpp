...
bool Tester::test(void) {
...

  {
    ...
    msg.pose.orientation = pairs_lib::AttitudeConverter(0, 0, 0).setHeading(2);
    ...
    if (std::abs(gt_tfed_pose->pose.position.x - pose_tfed->pose.position.x) > 0.1 ||
        ...
        std::abs(pairs_lib::geometry::sradians::diff(pairs_lib::AttitudeConverter(gt_tfed_pose->pose.orientation).getHeading(),
                                                   pairs_lib::AttitudeConverter(pose_tfed->pose.orientation).getHeading())) > 0.1) {
